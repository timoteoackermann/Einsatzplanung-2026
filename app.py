...
# --- POP-UP DIALOG FÜR ABWESENHEITSZEITRÄUME ---
    ...
    if not ma_absences:
        ...
        for idx, abw in enumerate(ma_absences):
            ...
            try:
                ...
                col_text.write(f"• Fehlerhaftes Datum")
...
# --- EXPANDER A: MITARBEITER STAMMDATEN MIT POP-UP TRIGGERN ---
with st.sidebar.expander("👥 Mitarbeiter-Pool verwalten"):
    ...
    for ma in MITARBEITER_POOL:
        ...
        # Pop-Up Trigger für Zeiträume
        if col_ma_abw.button("📅", key=f"btn_dialog_trigger_{ma}", help="Abwesenheitszeiträume hinterlegen"):
...
if modus == "👀 Kompakte Wochenübersicht":
    ...
    # HTML-Tabelle mit optimierten Schriftgrößen für 17-Zoll Monitore
    ...
    # Kombinierte Abwesenheiten in die Tabelle zeichnen
    ...
    if sorted_global_projects:
        ...
        for f in json_files:
            ...
            kw_part = base.replace("planung_", "").replace(".json", "")
            kw_part_clean = kw_part.replace("kw_", "").strip()
            try:
                kw_num = int(kw_part_clean)
            except ValueError:
                continue
...
# --- 8. MODUS B: BEARBEITUNGS-MODUS ---
    ...
    # --- SCHARFE KONFLIKTPRÜFUNG UNTER BERÜCKSICHTIGUNG VON HALBTAGEN ---
    konflikte_ganztag = []
    konflikte_halbtag = []
    ...
    for t in WOCHENTAGE_DATEN:
        ...
        # Generiere präzise Abwesenheitsliste für diesen Tag
        tag_absences = {} # ma -> typ
        ...
        # Vergleiche mit eingeteilten Mitarbeitern
        ...
        with st.expander(f"🏗️ {prj}", expanded=True):
            ...
            for i, t in enumerate(WOCHENTAGE_DATEN):
                ...
                with cols[i]:
                    ...
                    # Absicherung gegen Datenkonflikte (nur valide Mitarbeiter erlauben)
    ...
    st.caption("Zeigt automatisch alle fälligen Abwesenheiten dieser Woche aus den Stammdaten. Du kannst hier bei Bedarf weitere Personen händisch für den ganzen Tag hinzufügen.")
    ...
    for i, t in enumerate(WOCHENTAGE_DATEN):
        ...
        with cols_abw[i]:
            ...
            # Manuelle zusätzliche Abwesenheiten für diesen spezifischen Tag
