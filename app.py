    ...
    if os.path.exists(STAMMDATEN_FILE):
        try:
            with open(STAMMDATEN_FILE, "r", encoding="utf-8") as f:
                ...
                # Sicherheits-Check: Falls alte Datenstruktur vorliegt
                if "projekte" not in data:
                    data["projekte"] = default_data["projekte"]
                else:
                    # Formatiere alte Strings zu Dictionaries um, falls nötig
                    bereinigte_projekte = []
                    for p in data["projekte"]:
                        if isinstance(p, dict) and "nummer" in p and "name" in p:
                            bereinigte_projekte.append(p)
                        elif isinstance(p, str):
                            bereinigte_projekte.append({"nummer": "N/A", "name": p})
                    data["projekte"] = bereinigte_projekte
                    
                if "mitarbeiter" not in data:
                    data["mitarbeiter"] = default_data["mitarbeiter"]
                if "abwesenheiten" not in data:
                    data["abwesenheiten"] = default_data["abwesenheiten"]
    ...
    if os.path.exists(filename):
        try:
            with open(filename, "r", encoding="utf-8") as f:
                data = json.load(f)
                # Sicherheits-Check: Alte Projekte-Formate reparieren
                if "projekte" in data:
                    bereinigte_kw_projekte = []
                    for p in data["projekte"]:
                        if isinstance(p, str):
                            bereinigte_kw_projekte.append(p)
                        elif isinstance(p, dict) and "name" in p:
                            bereinigte_kw_projekte.append(f"{p.get('nummer', '')} - {p['name']}")
                    data["projekte"] = bereinigte_kw_projekte
                return data
        except Exception:
            pass
...
def create_pdf(data, wochentage_daten, kw_text, kw_datum_str):
    ...
    for prj in data.get("projekte", []):
        row = [Paragraph(str(prj), cell_bold_style)]
        for t in wochentage_daten:
            tag = t["name"]
            implicit_einsatz = data.get("einsatz", {}).get(prj, {}).get(tag, {})
...
with st.sidebar.expander("🏗️ Globaler Projekt-Pool (Stammdaten)"):
    ...
    for p in stammdaten.get("projekte", []):
        ...
        col_p_info.write(f"**{p.get('nummer', 'N/A')}** - {p.get('name', 'Ohne Name')}")
        if col_p_del.button("🗑️", key=f"del_global_p_{p.get('nummer', 'N/A')}"):
    ...
    if st.button("➕ Projekt in Stammdaten sichern"):
        if neue_p_nr and neue_p_name:
            if not any(p.get('nummer') == neue_p_nr for p in stammdaten["projekte"]):
...
with st.sidebar.expander("📅 Baustellen dieser Woche verwalten"):
    ...
    for prj in list(data.get("projekte", [])):
        ...
        if col_prj_del.button("🗑️", key=f"del_prj_{prj}"):
            ...
            if prj in data.get("einsatz", {}):
                del data["einsatz"][prj]
            save_data(kw_auswahl, data)
            st.rerun()
    st.markdown("**Projekt aus Stammdaten hinzufügen:**")
    global_p_options = [f"{p.get('nummer', 'N/A')} - {p.get('name', 'Unbekannt')}" for p in stammdaten.get("projekte", [])]
    available_p_options = [opt for opt in global_p_options if opt not in data.get("projekte", [])]
...
# --- 7. MODUS B: BEARBEITUNGS-MODUS ---
    ...
    for t in WOCHENTAGE_DATEN:
        tag = t["name"]
        tag_absences = {}
        for ma in data.get("abwesend", {}).get(tag, []):
        ...
        for abw_mitarbeiter, typ in tag_absences.items():
            if abw_mitarbeiter in belegte_mitarbeiter.get(tag, {}):
    ...
    for p_idx, prj in enumerate(data.get("projekte", [])):
    ...
    if "abwesend" not in data:
        data["abwesend"] = {n: [] for n in ["Montag", "Dienstag", "Mittwoch", "Donnerstag", "Freitag"]}
